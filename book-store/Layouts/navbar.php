<div class="container mt-5">
    <div class="row">
        <div class="col-lg-1">
            <i class="fa fa-bars mx-3" aria-hidden="true"></i>
        </div>
        <div class="col-lg-1">
            <h6 class="mx-3">HOMES</h6>
        </div>
        <div class="col-lg-1">
            <a class=" text-dark text-decoration-none " href="#">
                <h6 class="mx-3"><a class="text-decoration-none text-dark" href="#"> FEATURES</a></h6>
            </a>
        </div>
        <div class="col-lg-1">
            <a class=" text-dark text-decoration-none " href="#">
                <h6 class="mx-3"><a class="text-decoration-none text-dark" href="#"> PAGES</a></h6>
            </a>
        </div>
        <div class="col-lg-2">
            <img class="mx-4" src="../assets/images/logo-dark.png" width="150px" alt="image not found">
        </div>
        <div class="col-lg-1">
            <a class=" text-dark text-decoration-none " href="#">
                <h6 class="mx-3">SHOP</h6>
            </a>
        </div>
        <div class="col-lg-1">
            <a class=" text-dark text-decoration-none " href="#">
                <h6 class="mx-3">EVENT</h6>
            </a>
        </div>
        <div class="col-lg-1">
            <a class=" text-dark text-decoration-none " href="# ">
                <h6 class="mx-3">BLOG</h6>
            </a>
        </div>
        <div class="col-lg-1">
            <i class="fa fa-search mx-3 " aria-hidden="true"></i>
        </div>
        <div class="col-lg-1">
            <a class="text-dark d-flex" href="Add to card.html"> <i class="fa fa-shopping-cart mx-3" aria-hidden="true">
                    <h4 id="countCartItems"></h4>
                </i></a>
        </div>
        <div class="col-lg-1">
            <i class="fa fa-user-o mx-3" aria-hidden="true"></i>
        </div>
    </div>
</div>